...
client.defineJob({
  ...
  trigger: eventTrigger({
    ...
    schema: z.object({
      imageUrl: z
        ...
        .url()
        .default(
          "https://trigger.dev/blog/supabase-integration/postgres-meme.png"
        ),
    }),
  }),
  run: async (payload, io, ctx) => {
    ...
    if (typeof blipPrediction.output !== "string") {
      throw new Error(
        `Expected string output, got ${typeof blipPrediction.output}`
      );
    }
...
client.defineJob({
  ...
  run: async (payload, io, ctx) => {
    ...
    const all = await io.replicate.getAll(
